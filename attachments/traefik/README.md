...
middleware on `:80` and without it on `:81`, so the difference between them is
the attachment and nothing else. It refuses to measure until the middleware has
been seen to pass a benign request and block an attack, because a middleware
that is failing open — or blocking everything — would measure neither.
...
backend: a run serving blocked responses is measuring the cost of rejecting
...
more. That decision changes as the agent learns, so the same load can cost very
different amounts of work from one run to the next; a run showing no overhead
is usually one the agent stopped inspecting, not a fast one.
...
and labels a run that was no longer inspecting as measuring pass-through, not
inspection. Numbers carrying that warning say something about how the
